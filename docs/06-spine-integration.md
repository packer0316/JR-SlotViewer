...
### 1.1 為何選擇 `@esotericsoftware/spine-pixi`

| 考量面向 | 說明 |
|----------|------|
| **官方整合** | Spine 官方針對 PixiJS 8 維護的整合套件，API 穩定度與相容性有保障 |
| **Spine 4.x 完整功能** | 支援 Clipping Attachment、Custom Skin Batching、Mesh Deformation 等進階功能 |
| **渲染效能** | 同材質骨骼自動 Batching 為單次 Draw Call，與 PixiJS 渲染管線深度整合 |
| **版本同步** | 與 Spine Runtime 主版本同步更新，確保 `.skel` 檔格式相容 |
...
`SpineActor` 封裝單一 `spine.Spine` 實例的高階 API，隔離底層 `@esotericsoftware/spine-pixi` 的實作細節。每個 `SpineActor` 擁有獨立的 `AnimationState`，可自主控制播放行為。
...
│                                                      │
│  Spine 內部 Clipping 已由 spine-pixi runtime 處理     │
...
`@esotericsoftware/spine-pixi` 內建 Batching 機制：使用**相同材質**（Atlas Texture）的多個骨骼可合併為單次 Draw Call。
...
function validateSkelVersion(buffer: ArrayBuffer): boolean {
  ...
  if (major < 4) {
    showToast('error', `不支援的 Spine 版本 ${version}，需要 4.x 以上`);
    return false;
  }
...
`SpineActor` 介面作為抽象層，將底層 Spine Runtime 的 API 細節封裝在內部。當未來需要升級至新版 Spine Runtime（如 4.3、5.0）時，僅需修改 `SpineActor` 與 `SpineLoader` 的內部實作，對外介面保持不變：
...
└──────────────────────┬──────────────────────────┘
            ...
            SpineActor 抽象層
                       │
┌──────────────────────▼──────────────────────────┐
│  @esotericsoftware/spine-pixi 4.2.x             │
│  （內部實作，可替換為新版本）                       │
│                                                  │
...
若未來需要同時支援多個 Spine Runtime 版本（例如舊版 3.x 與新版 4.x 的 `.skel` 檔案），可透過 **Factory Pattern** 根據檔案版本選擇對應的 Runtime：

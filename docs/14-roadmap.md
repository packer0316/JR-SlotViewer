...
| PixiJS 整合 | PixiJS 7 + `@pixi-spine/all-3.8` 安裝與驗證 | Canvas 可渲染空白場景 |
...
| 任務 | 說明 | 產出 |
|------|------|------|
| cacheAsBitmap | 靜態圖層快取為紋理，減少 DrawCall | 效能提升 |
...
| 風險 | 機率 | 影響 | 緩解措施 |
|------|------|------|----------|
| **@pixi-spine/all-3.8 與 PixiJS 7 相容性** | 🟢 Low | 🟡 Medium | 已驗證的成熟版本組合（pixi.js@7.4.x + @pixi-spine/all-3.8 v4.0.6），於 Phase 0 進行整合驗證 |
| **WebGL2 瀏覽器相容性** | 🟢 Low | 🟢 Low | PixiJS 7 使用 WebGL2 renderer，瀏覽器支援成熟廣泛，無需 WebGPU |
| **大量 Spine 同時渲染效能** | 🟡 Medium | 🔴 High | 實作 cacheAsBitmap + Atlas Batching，並設定同時載入上限 |
...
Prob.   │                                          
────────┼──────────────────────────────────────────
Medium  │               • Zustand 過度   • Spine 效能
Prob.   │                 渲染                      
        │               • 依賴 Breaking                
────────┼──────────────────────────────────────────
Low     │               • WebGL2 相容    • @pixi-spine 相容
Prob.   │               • ZIP 效能       
        │               • Worker 瓶頸    
────────┼──────────────────────────────────────────
```
...
| PixiJS | ^7.4.x | 主要渲染引擎（WebGL2） |
| @pixi-spine/all-3.8 | ^4.0.x | Spine 3.8 Runtime 整合（已驗證 v4.0.6） |
| Zustand | ^5.x | 輕量狀態管理 |
